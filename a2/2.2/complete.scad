...
// Snap buckle connector with male and female parts.
// The male part has:
// - strap holder base
// - two flexible arms
// - solid triangle locking heads
// - center guide slot
//
// The female part has:
// - strap holder base
...
male_arm_body_length = male_arm_total_length - triangle_height;
...
// Male bag-style strap holder base
...
// Left triangle head.
// The vertical edge connects to the left arm.
// The point extends outward to the left.
module triangle_left_2d(width, height) {
    polygon(points=[
        [0, 0],
        [0, height],
        [-width, 0]
...
// Right triangle head.
// The vertical edge connects to the right arm.
// The point extends outward to the right.
module triangle_right_2d(width, height) {
    polygon(points=[
        [0, 0],
        [0, height],
        [width, 0]
...
module triangle_left_head(width, height, thickness) {
    linear_extrude(height = thickness)
        triangle_left_2d(width, height);
}


module triangle_right_head(width, height, thickness) {
    linear_extrude(height = thickness)
        triangle_right_2d(width, height);
}
...
module male_connector() {
    ...
    difference() {
        ...
        union() {
            ...
            // Left triangle locking head
            translate([
                -male_base_width/2 + male_arm_width,
                male_base_depth + male_arm_body_length - male_head_overlap,
                0
            ])
                triangle_left_head(
                    triangle_base,
...
module female_connector() {
    ...
    union() {
        ...
        difference() {
            ...
            // Solid base behind the receiver
        ...
        difference() {
            ...
            // Internal cutout where the male connector slides in
            translate([
